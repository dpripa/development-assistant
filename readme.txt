...
= Features =
- **Status and Actions in the Toolbar:** Review WP Debug and Support User status and run their available actions from the Development Assistant menu in the WordPress toolbar, both in wp-admin and on the front end.
- **Debugging Made Easy:** Enable WP_DEBUG, WP_DEBUG_LOG, and WP_DEBUG_DISPLAY modes directly from the WordPress admin panel without the need to manually edit the wp-config.php file. Configuration changes are validated, backed up, and rolled back if they cannot be verified; unsupported syntax or filesystem restrictions are reported without claiming success.
